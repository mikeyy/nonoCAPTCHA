...
class DeepSpeech(object):
    MODEL_DIR = settings["deepspeech"]["model_dir"]
...
class Sphinx(object):
    MODEL_DIR = settings["pocketsphinx"]["model_dir"]
...
class Amazon(object):
    ACCESS_KEY_ID = settings["amazon"]["secret_key_id"]
    SECRET_ACCESS_KEY = settings["amazon"]["secret_access_key"]
    REGION_NAME = settings["amazon"]["region"]
    S3_BUCKET = settings["amazon"]["s3_bucket"]
...
class Azure(object):
    SUB_KEY = settings["azure"]["api_subkey"]
